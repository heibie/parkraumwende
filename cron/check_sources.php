 ...
 *      nach main committet (rein informative Ampel auf quellen.html, ändert keine
 *      Chart-Daten, daher unkritisch für Auto-Commit).
 ...
 *      zu berechnen - die Umrechnung Portal-Rohdaten -> unser Jahresformat ist pro Quelle
 *      unterschiedlich (siehe scripts/check_sources.py bzw. README), und falsch generierte
 *      Werte sollen nicht ungeprüft in die Diagramme wandern. Die CSV-Aktualisierung bleibt
 *      ein manueller Schritt (siehe README, "CSV-Daten aktualisieren").
 *
 * "heuristic"-Quellen (keine Live-API) werden ohne Netzwerk-Call geprüft: lokales
 * Änderungsdatum (aus dem letzten Stand übernommen, da hier kein Git-Zugriff besteht)
 * gegen den bekannten Turnus. Das lokale Datum wird nur dann aktuell gehalten, wenn
 * jemand lokal scripts/check_sources.py laufen lässt (das hat Git-Zugriff) und committet -
 * z.B. im Rahmen eines manuellen CSV-Updates.
 */
...
try {
    ...
    foreach ($registry['categories'] as $cat) {
        foreach ($cat['sources'] as $src) {
            ...
            if ($method === 'cron') {
                $result = ['status' => 'ok', 'detail' => $src['check']['note'] ?? ''];
            } elseif ($method === 'ckan') {
    ...
    if (!$statusUnchanged) {
        // Direkt auf Festplatte schreiben - sofort sichtbar ohne Deploy-Umweg (wie latest.json)
...
function issue_body(array $c): string
{
    ...
    return "Automatischer Wochen-Check hat eine Änderung an der Quelle **{$c['title']}** erkannt.\n\n"
         ...
         . "Bitte in der [Quellen-Übersicht](https://data.parkraumwende.de/quellen.html) gegenprüfen "
         . "und die betroffene CSV bei Bedarf manuell aktualisieren (siehe README, Abschnitt "
         . "\"CSV-Daten aktualisieren\"). Dieses Issue danach schließen.\n\n"
...
function check_ckan(string $dataset, array $prev): array
{
    ...
    $ctx = stream_context_create([
        'http' => ['timeout' => 20, 'header' => "User-Agent: parkraumwende-source-check\r\n"],
    ]);
    ...
    // "GeoPortal Last Modified Date" (nur bei Geoportal-Datensätzen) ist das echte
    // Änderungsdatum der Geodaten. CKANs eigenes metadata_modified/resource.last_modified
    // ist bei diesen Datensätzen unbrauchbar - tägliches Neu-Harvesting mit neuen
    // Resource-IDs, auch ohne echte Datenänderung.
...
function check_heuristic(array $src, array $prev): array
{
    ...
    $intervalDays = null;
    if (str_contains($turnus, '~5 Jahre')) {
        $intervalDays = 365 * 5;
    } elseif (str_contains($turnus, 'jährlich')) {
        $intervalDays = 365;
    }
